...
const seedDP = async () => {
    try {
        ...
        // Path to the DP JSON file
        const problemsPath = path.join(__dirname, 'dynamic_programming_problems.json');
        ...
        // Delete existing DP problems to avoid duplicates
        await CodingProblem.deleteMany({ pattern: "Dynamic Programming" });
        console.log('Cleared existing "Dynamic Programming" problems.');
        ...
        uniqueProblems = uniqueProblems.map(p => {
            if (existingSlugsInDB.has(p.slug)) {
                ...
                return { ...p, slug: `${p.slug}-dp` };
            }
            return p;
        });

        await CodingProblem.insertMany(uniqueProblems);
        console.log(`Successfully seeded ${uniqueProblems.length} Dynamic Programming problems!`);
