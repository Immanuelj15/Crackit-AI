...
const seedHashing = async () => {
    try {
        ...
        // Path to the Hashing JSON file
        const problemsPath = path.join(__dirname, 'hashing_problems.json');
        ...
        // Delete existing Hashing problems to avoid duplicates/stale data
        await CodingProblem.deleteMany({ pattern: "Hashing" });
        console.log('Cleared existing "Hashing" problems.');
        ...
        const problemsWithSlugs = problems.map(problem => {
            ...
            return {
                ...problem,
                slug,
                pattern: 'Hashing', // Set correct pattern
        ...
        uniqueProblems = uniqueProblems.map(p => {
            if (existingSlugs.has(p.slug)) {
                console.log(`Slug collision for "${p.title}". Renaming slug to "${p.slug}-h".`);
                return { ...p, slug: `${p.slug}-h` };
            }
            return p;
        });

        await CodingProblem.insertMany(uniqueProblems);
        console.log(`Successfully seeded ${uniqueProblems.length} Hashing problems!`);

        // Verification
        const count = await CodingProblem.countDocuments({ pattern: 'Hashing' });
        console.log(`Database now contains ${count} Hashing problems.`);
