...
const seedBinarySearch = async () => {
    try {
        await mongoose.connect(process.env.MONGO_URI);
        console.log('MongoDB Connected');

        // Path to the Binary Search JSON file
        const problemsPath = path.join(__dirname, 'binary_search_problems.json');
        ...
        // Delete existing Binary Search problems to avoid duplicates/stale data
        await CodingProblem.deleteMany({ pattern: "Binary Search" });
        console.log('Cleared existing "Binary Search" problems.');
        ...
        const problemsWithSlugs = problems.map(problem => {
            ...
            return {
                ...problem,
                slug,
                pattern: 'Binary Search', // Set correct pattern
        ...
        // Check for existing slugs in DB (from OTHER patterns)
        // We already deleted "Binary Search" pattern problems, so any collisions now are with other patterns
        ...
        uniqueProblems = uniqueProblems.map(p => {
            if (existingSlugs.has(p.slug)) {
                console.log(`Slug collision for "${p.title}". Renaming slug to "${p.slug}-bs".`);
                return { ...p, slug: `${p.slug}-bs` };
            }
            return p;
        });

        await CodingProblem.insertMany(uniqueProblems);
        console.log(`Successfully seeded ${uniqueProblems.length} Binary Search problems!`);

        // Verification
        const count = await CodingProblem.countDocuments({ pattern: 'Binary Search' });
        console.log(`Database now contains ${count} Binary Search problems.`);
