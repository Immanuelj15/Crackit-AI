...
const seedPrefixSum = async () => {
    try {
        await mongoose.connect(process.env.MONGO_URI);
        console.log('MongoDB Connected');

        // Path to the Prefix Sum JSON file
        const problemsPath = path.join(__dirname, 'prefix_sum_problems.json');
        ...
        // Delete existing Prefix Sum problems to avoid duplicates/stale data
        await CodingProblem.deleteMany({ pattern: "Prefix Sum" });
        console.log('Cleared existing "Prefix Sum" problems.');
        ...
        const problemsWithSlugs = problems.map(problem => {
            ...
            return {
                ...problem,
                slug,
                pattern: 'Prefix Sum', // Set correct pattern
        ...
        uniqueProblems = uniqueProblems.map(p => {
            if (existingSlugs.has(p.slug)) {
                console.log(`Slug collision for "${p.title}". Renaming slug to "${p.slug}-ps".`);
                return { ...p, slug: `${p.slug}-ps` };
            }
            return p;
        });

        await CodingProblem.insertMany(uniqueProblems);
        console.log(`Successfully seeded ${uniqueProblems.length} Prefix Sum problems!`);

        // Verification
        const count = await CodingProblem.countDocuments({ pattern: 'Prefix Sum' });
        console.log(`Database now contains ${count} Prefix Sum problems.`);
