...
const seedStackQueue = async () => {
    try {
        await mongoose.connect(process.env.MONGO_URI);
        console.log('MongoDB Connected');

        // Path to the Stack & Queue JSON file
        const problemsPath = path.join(__dirname, 'stack_queue_problems.json');
        ...
        // Delete existing Stack & Queue problems to avoid duplicates/stale data
        await CodingProblem.deleteMany({ pattern: "Stack & Queue" });
        console.log('Cleared existing "Stack & Queue" problems.');
        ...
        const problemsWithSlugs = problems.map(problem => {
            ...
            return {
                ...problem,
                slug,
                pattern: 'Stack & Queue', // Set correct pattern
        ...
        uniqueProblems = uniqueProblems.map(p => {
            if (existingSlugs.has(p.slug)) {
                console.log(`Slug collision for "${p.title}". Renaming slug to "${p.slug}-sq".`);
                return { ...p, slug: `${p.slug}-sq` };
            }
            return p;
        });

        await CodingProblem.insertMany(uniqueProblems);
        console.log(`Successfully seeded ${uniqueProblems.length} Stack & Queue problems!`);

        // Verification
        const count = await CodingProblem.countDocuments({ pattern: 'Stack & Queue' });
        console.log(`Database now contains ${count} Stack & Queue problems.`);
