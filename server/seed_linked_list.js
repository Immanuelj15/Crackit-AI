...
const seedLinkedList = async () => {
    try {
        await mongoose.connect(process.env.MONGO_URI);
        console.log('MongoDB Connected');

        // Path to the Linked List JSON file
        const problemsPath = path.join(__dirname, 'linked_list_problems.json');
        ...
        // Delete existing Linked List problems to avoid duplicates/stale data
        await CodingProblem.deleteMany({ pattern: "Linked List" });
        console.log('Cleared existing "Linked List" problems.');
        ...
        const problemsWithSlugs = problems.map(problem => {
            ...
            return {
                ...problem,
                slug,
                pattern: 'Linked List', // Set correct pattern
        ...
        uniqueProblems = uniqueProblems.map(p => {
            if (existingSlugs.has(p.slug)) {
                console.log(`Slug collision for "${p.title}". Renaming slug to "${p.slug}-ll".`);
                return { ...p, slug: `${p.slug}-ll` };
            }
            return p;
        });

        await CodingProblem.insertMany(uniqueProblems);
        console.log(`Successfully seeded ${uniqueProblems.length} Linked List problems!`);

        // Verification
        const count = await CodingProblem.countDocuments({ pattern: 'Linked List' });
        console.log(`Database now contains ${count} Linked List problems.`);
