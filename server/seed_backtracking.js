...
const seedBacktracking = async () => {
    try {
        await mongoose.connect(process.env.MONGO_URI);
        console.log('MongoDB Connected');

        // Path to the Backtracking JSON file
        const problemsPath = path.join(__dirname, 'backtracking_problems.json');
        ...
        // Delete existing Backtracking problems to avoid duplicates
        await CodingProblem.deleteMany({ pattern: "Backtracking" });
        console.log('Cleared existing "Backtracking" problems.');
        ...
        uniqueProblems = uniqueProblems.map(p => {
            if (existingSlugsInDB.has(p.slug)) {
                ...
                return { ...p, slug: `${p.slug}-bt` };
            }
            return p;
        });

        await CodingProblem.insertMany(uniqueProblems);
        console.log(`Successfully seeded ${uniqueProblems.length} Backtracking problems!`);
